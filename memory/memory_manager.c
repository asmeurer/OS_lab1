 ...
 * @param num_pages The number of pages to be allocated for the allocation
 * @return Returns the page table id of the allocated pagetable, or an error
 */
 ...
 * @param page_table_index The page table index of the page table to be dealloc
...
int dealloc_pt (int page_table_index){
	...
	for(i = 0; i < MAX_PAGES_PER_PROCESS; i++){
		...
		if(page_tables[page_table_index][i].bits & BMV_BITMASK) {
			error = set_back_addr_empty( page_tables[page_table_index][i].back_addr );
 ...
 * @return Returns the index of the free frame in physical memory
 */
byte lru_lookup(){
	byte i;
...
/**
 * Set the address addr to full.  Returns errors if the memory is
...
/**
 * A function to fill the physical memory from a page table. The page table
 * must have at least the USER_PHY_MEM_NUM_FRAMES allocated.
 * @param page_table_id The page table to fill physical memory
 * @return Returns an error code
 */
...
/**
 * A function create a page fault to allocate memory for the page needed.
 * @param page_table_index The page table index of the page fault
 * @param page_num The page number of the page fault
 * @return Returns an error code
 */
int page_fault(int page_table_index, int page_num){
	...
	if (victimized != null){
		...
		if(!(victimized->bits & BMV_BITMASK)){
			...
			error = set_back_addr_full(victimized->back_addr);
			if(error != ERROR_SUCCESS) {
				return error;
			}
...
/**
 * A function to set the LRU counter to overflow on the next page access
 */
void set_LRU_overflow(){
	global_LRU_counter = 0xFFFFFFFE;
}

/**
 * A function to check if the global LRU counter is going to overflow. If
 * it is, reset the LRU global counter as well as each LRU in LRU table.
 */
void checkoverflow(){
	int i;
...
/**
 * A function stub that is going to write to the backing store.
 */
